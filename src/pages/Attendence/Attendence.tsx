...
const Attendences = () => {
  ...
  const getStudents = async () => {
    const s = await mainAxios.get(
      `/api/v1/student/get-student-year-info-by?bySchoolYearClassId=${idClass}`
    );
    setStudent(s.data);
  };
  useEffect(() => {
  ...
  return (
    <div className="attendances ">
      ...
      <div
        className={`${attendence !== 'attendance-by-day' ? 'hiddens' : 'attendance-by-day'}`}
      >
        <div style={{ display: 'flex', padding: '16px' }}>
          <div style={{ marginRight: '14px' }}>
            <Select
              ...
              defaultValue={
                getClass
                  ? options.find((e) => e.value == parseInt(getClass))?.value
                  : options[0].value
              }
      ...
      <div
        className={`${attendence !== 'attendance-by-month' ? 'hiddens' : 'attendance-by-month'}`}
      >
        <div style={{ display: 'flex', padding: '16px' }}>
          <div className="class">
            <div
            ...
            <div className={classNameClass}>
              {AllClasses.map((c) => (
                <div
                  onClick={() => choseClass(c.className)}
                  key={c.classId}
                  className="classItem"
                >
                  {c.className}
