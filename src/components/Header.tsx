...
const Header = (props: {
  ...
  const [idYear, setIdYear] = useState(parseInt(getYear!) ?? 1);
  const optionsYear = schoolYears.map(y => ({
    value: y.id,
    label: y.startSem1 + '-' + y.end
  }));
  ...
  return (
    ...
    >
      <div className="flex flex-grow items-center justify-between py-4 px-4 shadow-2 md:px-6 2xl:px-11">
        ...
        <div className='ml-3'>
          <Select className='w-25 h-10' options={optionsYear} defaultValue={getYear ? optionsYear.find(e => e?.value == parseInt(getYear))?.value : optionsYear[0]?.value} onChange={handleChange} />
        </div>
